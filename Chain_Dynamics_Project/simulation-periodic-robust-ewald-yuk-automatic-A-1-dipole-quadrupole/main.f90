...
!!!ASSIGNMENT OF IMAGE HALF-CELL POSITION AND VELOCITY!!!
!  call assign_side_wall_images(y,dy,y_sw,dy_sw)
...
!!!BACK ASSIGNING VELOCITIES TO MAIN HALF-CELL PARTICLES
!  call back_assign(y,dy,y_sw,dy_sw)
...
subroutine combined_VEL(i,j,V,relpos)
  ...
  integer :: i,j
!!!real*8 :: V1(2), V2(2),
  real*8 :: V(2),relpos(2),v_s(2)
  !real*8 :: alpha
  real*8 :: beta(2)
  !alpha = 0.85d0!1.d0!0.75d0 !1.d0 !0.5d0!1.d0
  beta(1) = 1.d0!!! 0.75d0
  beta(2) = 0.d0
...
subroutine linear_varying_array_1(y_new)
  use main_module
  use two_particles_para
  implicit none
  integer :: i,j,k,l,p,particle_count,num_extra_spaces
  real*8 :: y(2*numpar),y_new(2*numpar-2*pair_num+numpar_extra),free_space_x, space_rem, drho_new2(2)
  ...
  read(*,*)drho(1)
  read(*,*)drho(2)
  write(*,*)"Interparticle distance 1",drho(1),drho(2)

  read(*,*)drho_2(1)
  read(*,*)drho_2(2)
  write(*,*)"Interparticle distance 2",drho_2(1),drho_2(2)

  read(*,*)num_high
  write(*,*)"Number of high density particles=",num_high
     
  if(drho_2(1) .lt. 1.d0)then
     write(*,*)"Warning: Particle spacing less than diameter"
     STOP
  end if

!!!Placing particles in two different spacing arrangements

  middle_lim = numpar - num_high
  !right_lim = (numpar - 2*pair_num)/2 !- numpar_extra)/2

  !write(*,*)"left_lim=",left_lim
  !write(*,*)"middle_lim=",middle_lim
  !write(*,*)"right_lim=",right_lim
  
  do i=1,middle_lim
     y_new(2*i-1) = dble( (i-1)+0.5)*drho(1)
     y_new(2*i) = 0.5d0*Ly
  end do

  do i= middle_lim + 1,numpar
     y_new(2*i-1) = middle_lim*drho(1) + dble((l-1)+0.5)*drho_2(1)
     y_new(2*i) = 0.5d0*Ly
     l = l + 1
  end do
 
  ! Write the arrangement on screen
  
  do i=1,numpar
     write(*,*)y_new(2*i-1),y_new(2*i)
  end do
  
end subroutine linear_varying_array_1
...
subroutine linear_varying_array(y_new)
  ...
  implicit none
  integer :: i,j,k,l,p,particle_count,num_extra_spaces
  real*8 :: y(2*numpar),y_new(2*numpar),free_space_x, space_rem, drho_new2(2)
  integer :: left_lim, middle_lim1, middle_lim2, right_lim
  particle_count=0
  k=1
  l=1
  p=1
...
!!!No. of extra particles allowed with given drho & drho_new & dia
  ...
  space_rem = free_space_x - pair_num * drho_new(1)
  drho_new2(1) = space_rem/pair_num
  drho_new2(2) = drho(2)
  
  write(*,*)"Free space created=",free_space_x
  write(*,*)"New particle separation=",drho_new(1)
  write(*,*)"Space remaining= ",space_rem
  write(*,*)"New 2nd particle separation=",drho_new2
...
!!!Shifting particle position array to accomodate newer particles
  ...
  left_lim = (numpar - 2*pair_num )/2 !- numpar_extra)/2
  middle_lim1 = pair_num !!! + numpar_extra
  middle_lim2 = pair_num
  right_lim = (numpar - 2*pair_num)/2 !- numpar_extra)/2
  ...
  do i=left_lim + 1,left_lim + middle_lim1
     y_new(2*i-1) = left_lim*drho(1) + dble( (l-1)+0.5)*drho_new(1)
     y_new(2*i) = 0.5d0*Ly
     l = l + 1
  end do

  do i=left_lim + middle_lim1 + 1, left_lim + middle_lim1 + middle_lim2
     y_new(2*i-1) = left_lim*drho(1) + middle_lim1*drho_new(1) + dble( (p-1)+0.5)*drho_new2(1)
     y_new(2*i) = 0.5d0*Ly
     p = p + 1
  end do
  
  do i=left_lim + middle_lim1 + middle_lim2 + 1,left_lim + middle_lim1 + middle_lim2 + right_lim
     y_new(2*i-1) = left_lim*drho(1) + middle_lim1*drho_new(1) + middle_lim2*drho_new2(1) + dble( (k-1)+0.5)*drho(1)
     y_new(2*i) = 0.5d0*Ly
     k = k + 1
  end do
