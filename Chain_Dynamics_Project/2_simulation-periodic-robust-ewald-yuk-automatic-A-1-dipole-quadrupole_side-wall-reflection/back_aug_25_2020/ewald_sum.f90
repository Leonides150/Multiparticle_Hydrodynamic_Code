!!!!NOTE: GAMMA(1)=GAMMA(2)=1.d0/ HENCE THE 1/GAMMA(m) TERM 
...
subroutine first_sum(ro,sum1)
  ...
  complex*16 :: grad_gam(2),product_a(2),sum1_a(2)
  integer :: ii,jj
  character(5) :: flag
  ...
  do ii= -n1max,n1max          
     do jj=-m1max,m1max
        ...
        if(flag_self .AND. (ii.eq.0 .AND. jj.eq.0) )then
           ...
           if(rel_ro_mag .le. (10.d0*sigma))then!!!!CHANGED DURING CONVERGENCE TESTS FROM (3.d0*sigma)
              ...
              prod(2) = (grad_gam(2)*phi_min_np) + (grad_phinp(2)&
                   *incom_gamma)
              product_a(1) = (grad_gam_a*phi_min_np)+(grad_phinp_a&
                   *incom_gamma)
              product_a(2) = (grad_gam_a*phi_min_np)+(grad_phinp_a&
                   *incom_gamma)
              !product = phi_min_np*incom_gamma
              sum1(1) = sum1(1) + prod(1)
              sum1(2) = sum1(2) + prod(2)
              
              sum1_a(1) = sum1_a(1) + product_a(2)
              sum1_a(2) = sum1_a(2) + product_a(2)
