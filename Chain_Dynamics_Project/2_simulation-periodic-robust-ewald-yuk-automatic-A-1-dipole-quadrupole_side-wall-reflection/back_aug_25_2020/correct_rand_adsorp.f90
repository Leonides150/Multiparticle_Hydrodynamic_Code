...
!!!Adhoc Concentraton Region!!!
!     area_frac = (i*0.5233)/(Lx*0.5d0*Ly)
!     if(area_frac .le. 0.07d0)then
!        continue
!     else
!        frac_x=0.30d0!0.15d0
!        shift_frac=0.25d0
!        frac_y=0.5d0*0.35d0!*0.33d0
!     end if
...
!!!rel_dist = sqrt(rel_cood(1)**2 + rel_cood(2)**2)
     ...
     if(rel_dist .lt. dia .OR. y(2*i) .ge. (0.5d0*Ly-0.5d0*dia) .OR. y(2*i) .le. (0.5d0*dia) )then
        overlap=.TRUE.
        return
     end if
     overlap=.FALSE.
...
subroutine random_cood(rand_cood)
  ...
  integer :: i
 !!! real*8 :: frac_x=0.35d0, frac_y=0.5d0, shift_frac=0.25d0 !!! frac=1.d0, shift_frac=0.d0 !!! frac=0.5d0, shift_frac=0.25d0
   
  do i = 1,2
     
     call random_number(x)
     if(i .eq. 1)then
        rand_cood(i) = x*Lx*frac_x + shift_frac*Lx
     else
        rand_cood(i) = x*Ly*frac_y
     endif
