...
subroutine importbeadcoords(y,dy)  
  ...
  real*8 :: a1,a2,a3,a4     !dummy variables

  open(2,file='A00600.dat',status='unknown')
  
  do k=1,numpar
     !To restart from already created files
     read(2,*)a1,y(2*k-1), y(2*k),dy(2*k-1),dy(2*k),a2,a3,a4
...
!!!call test_pos(y)
  ...
  open(3,file='evolution_time.dat',status='unknown')
...
!!!     y=yo; t=t+dt
     call rk4a(y,2*numpar,t,dt,err,derives)
     write(3,*)l,t
...
subroutine derives(t,y,dy,nn)
  ...
  real*8 :: y_sw(4*numpar),dy_sw(4*numpar)
  ...
  do i=1,2*numpar
     ...
     do j=1,2*numpar
        ...
        dy_sw(2*i-1)=dy_sw(2*i-1)+V(1)
        dy_sw(2*i)=dy_sw(2*i)+V(2)
 
     enddo
     !write(*,*)'pos',i,y_sw(2*i-1),y_sw(2*i)
     !write(*,*)'###','vel',dy_sw(2*i-1),dy_sw(2*i)
  enddo
...
!!!  write(*,*)'derives'
!  do i=1,2*numpar
!     write(*,*)i,dy_sw(2*i-1),dy_sw(2*i)
!  end do
  
!!!TRAILING END ADLUSTMENT
  call trailing_end_potential(y,dy) !!!SWITCH OFF FOR FULL CELL SIMS
  
!!!Leading end for Two Piston setup
  call leading_end_potential(y,dy)
...
!!!MAKING SELF-TERM ZERO!!!
  ...
  V(1)=V_d(1)+beta(1)*V_q(1)+V2(1)+v_s(1)
!!!v_s added to only x component since along x dir!!!
  V(2)=V_d(2)+beta(2)*V_q(2)+V2(2)+alpha*v_s(2)
...
!!!No. of extra particles allowed with given drho & drho_new & dia
  free_space_x = ( (2*pair_num) + 1 ) * drho(1)  
  drho_new(1) = free_space_x/(numpar_extra + 2*pair_num + 1)
  drho_new(2) = drho(2)
...
!!!FIVE PARTICLE PLACEMENT!!!
  ...
  num=numpar/2
  do i=1,numpar
     y(2*i-1) = Lx*0.5d0 - dble(i)*dia + 2.d0*dia
     y(2*i) = Ly*0.5d0 - dble(i)*dia
  end do

!  do i=num+1,numpar
!     y(2*i-1) = Lx*0.5d0 + dble(i)
!     y(2*i) = Ly*0.25d0
!  end do
