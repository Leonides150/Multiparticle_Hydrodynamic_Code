...
subroutine random_cood(rand_cood)
  ...
  integer :: i
  real*8 :: frac=0.877d0, shift_frac=0.d0 ! frac=0.5d0, shift_frac=0.25d0 !!! frac=1.d0, shift_frac=0.d0 !!! frac=0.5d0, shift_frac=0.25d0
   
  do i = 1,2
     
     call random_number(x)
     if(i .eq. 1)then
        rand_cood(i) = x*Lx*frac + shift_frac*Lx
     else
        rand_cood(i) = x*Ly
     endif
